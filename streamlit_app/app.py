...
# Main tabs
tab1, tab2 = st.tabs(["Resume Scoring", "Find Best Jobs"])

with tab1:
    # Resume upload
    st.write("### Upload Resume")
    resume_file = st.file_uploader("Upload your resume (PDF or DOCX)", type=["pdf", "docx"], key="resume_scorer")
...
with tab2:
    st.write("### Find Your Best Job Matches")
    resume_file_matcher = st.file_uploader("Upload your resume (PDF)", type=["pdf"], key="resume_matcher")

    if resume_file_matcher:
        st.success(f"Resume uploaded: {resume_file_matcher.name}")

        if st.button("Find Best Jobs"):
            with st.spinner("Searching for the best jobs for you..."):
                top_jobs = asyncio.run(find_best_jobs(resume_file_matcher))

            st.write("### Top 5 Job Matches")
            if top_jobs:
                for i, job in enumerate(top_jobs, 1):
                    st.write(f"**{i}. {job['job']['title']} at {job['job']['company']}**")
                    st.write(f"   **Location:** {job['job']['location']}")
                    st.write(f"   **Match Score:** {job['score']:.1f}/100")
                    with st.expander("View Job Description"):
                        st.write(job['job']['description'])
                    st.write("---")
            else:
                st.warning("Could not find any job matches. Please try again later.")
